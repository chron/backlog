...
describe("WeekendScreen", () => {
  it("renders the calendar break, three early plans, and their outcomes", () => {
    ...
    const markup = renderToStaticMarkup(
      <WeekendScreen dispatch={() => undefined} run={run} nodeId="weekend-1" />,
    );
  ...
  it("adds the boss-relevant One Last PR plan only at the final Weekend", () => {
    ...
    const markup = renderToStaticMarkup(
      <WeekendScreen dispatch={() => undefined} run={state.run} nodeId="weekend-2" />,
    );
